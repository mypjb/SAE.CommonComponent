...
export default (props) => {
  ...
  const [state, setState] = useState({ ...defaultState, params: { appId, environmentId: defaultEnvId } });
  const { paging } = state;
  ...
  const handleSkipPage = (pageIndex, pageSize) => {
    dispatch({
      type: "appConfigReference/paging",
      payload: {
        pageIndex,
        pageSize,
        ...state.params,
  ...
  props.okCallback((close) => {
    dispatch({
      type: "appConfigReference/reference",
      payload: {
        data: { configIds, appId },
        callback: close
      }
    });
    return false;
